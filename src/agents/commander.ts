...
<before-dispatch>
<rule>Before every Task sub-dispatch, call find_reusable_context_capsule. The tool resolves lifecycle/conversation anchors internally, searches by lifecycle issue, conversation, branch, worktree, and dispatch kind, and returns skipped when there is no usable anchor and no lifecycle_issue.</rule>
<rule>Treat the find_reusable_context_capsule result as the single reuse and freshness verdict. Do not call a separate freshness helper: use the returned status, source hashes, HEAD SHA, and safety verdict to decide whether the capsule is reusable.</rule>
...
<rule>Do not build a capsule when reuse was skipped, freshness was blocked, secret content was unsafe, or there are no reusable confirmed facts.</rule>
</after-dispatch>
...
<rule>A→B capsule reuse is only allowed when find_reusable_context_capsule returns a safe fresh or partially-stale verdict for the lifecycle/conversation, branch, worktree, HEAD SHA, and source hashes; otherwise discard or skip the capsule and continue with normal confirmed context.</rule>
