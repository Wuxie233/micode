...
- channel selection reuses existing `Interactive Question Tools` table: ≤3 plain chat numbered; ≥4 octto。少量选择用当前聊天编号提问，大批量或高信息密度问题交给 Octto 批处理。
...
- `/all-init`、`/all-rebuild`、`/all-status` 是 bootstrap-only flows；三条命令不需要 lifecycle ownership preflight，不启动 lifecycle，不创建 GitHub issue，不创建 lifecycle branch，也不运行 ownership preflight。它们不会调用 `lifecycle_start_request`，只做知识库 bootstrap / status / rebuild 边界工作。
